...
fun part1(rawInput: String) {
    val wireOne = Wire.parse(rawInput.lines()[0])
    val wireTwo = Wire.parse(rawInput.lines()[1])

    println("All intersections: ${wireOne.intersect(wireTwo)}")
    println("Distance from origin to closest intersection: ${wireOne.distanceToClosestIntersectionWith(wireTwo)}")
}

class Wire(turns: List<Point>) {
    ...
    private fun Point.distanceFromOrigin() = abs(0 - this.x) + abs(0 - this.y)

    fun distanceToClosestIntersectionWith(other: Wire) = intersect(other).first().distanceFromOrigin()

    companion object {
        fun parse(input: String, origin: Point = Point(0, 0)): Wire {
