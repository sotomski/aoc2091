...
fun part2(rawInput: String) {
    ...
    val intersections = wireOne.intersect(wireTwo)

    val stepsOne = intersections.map { wireOne.stepsTo(it) }
    val stepsTwo = intersections.map { wireTwo.stepsTo(it) }

    println("Intersections found: $intersections")

    println("Steps to get to each intersection on wire 1: $stepsOne")
    println("Steps to get to each intersection on wire 2: $stepsTwo")

    val summedSteps = stepsOne.zip(stepsTwo)
        .map { pair -> pair.first + pair.second }

    println("Sums for both wires: $summedSteps with smallest being: ${summedSteps.min()}")
}